...
window.addEventListener('load', () => {
  ...
  const sectionTypeArray = [
    'modeofattendancecode_s-full-time',
    'modeofattendancecode_s-part-time',
    'januarystart_s-y',
  ...
  const clearLocalStorage = () => {
    ...
    for (const key in localStorage) {
      if (key.startsWith('pg-search-for-')) {
  ...
  const handleSectionTypeStorage = () => {
    ...
    inputElements.forEach(el => {
      ...
      if (facetId && sectionTypeArray.includes(facetId)) {
        ...
        localStorage.setItem(`pg-search-for-${el.id}`, facetId);
  ...
  const addCloseBtnToInput = () => {
    if (keywordSearchForm.value.trim() !== "") {
      if (!document.querySelector(".t4-close-search")) {
        ...
        buttonElement.innerHTML = `<span class="search-close" style="max-height:67px;"/>`;
      ...
      document.querySelector(".t4-close-search")
        ? document.querySelector(".t4-close-search").remove()
        : null;
    }
  };
